...
Goals:
- keep the overall structure of the uploaded script
- do not stop the whole run because of per-example errors
- print errors instead of raising whenever practical
...
- if graph_source_rag fails, print the error and fall back to context-only prompting
...
def log_error(where: str, err: Union[str, Exception], extra: str = "") -> None:
    if isinstance(err, Exception):
        msg = f"{type(err).__name__}: {err}"
    else:
        msg = str(err)
    if extra:
        print(f"[error][{where}] {msg} | {extra}")
    else:
...
def parse_dataset_spec(spec: str) -> Optional[Tuple[str, Optional[int]]]:
    try:
        spec = spec.strip()
        ...
        log_error("parse_dataset_spec", e, extra=f"spec={spec}")
        return None
...
def parse_datasets_string(datasets_str: str) -> List[Tuple[str, Optional[int]]]:
    ...
    if not datasets_str or not datasets_str.strip():
        log_error("parse_datasets_string", "empty --datasets string")
        return parsed

    for spec in datasets_str.split("|"):
        spec = spec.strip()
        if not spec:
            continue
        item = parse_dataset_spec(spec)
        if item is not None:
            parsed.append(item)

    if not parsed:
        log_error("parse_datasets_string", "no valid datasets found")
...
def sample_streamed_dataset(
    ...
    log_info(f"Streaming dataset: {dataset_name} (split={split}, {limit_str})")
    rows = []
    try:
        ...
        log_error("sample_streamed_dataset", e, extra=f"dataset={dataset_name}")

    ds = Dataset.from_list(rows)
    log_info(f"  -> Collected {len(ds)} rows")
    return ds
...
def build_corpus(
    ...
    if not datasets_list:
        log_error("build_corpus", "No data collected from any dataset")
        return Dataset.from_list([])

    combined = datasets_list[0] if len(datasets_list) == 1 else concatenate_datasets(datasets_list)
    combined = combined.shuffle(seed=42)
    log_info(f"Final combined corpus size: {len(combined)}")
    return combined
...
def clean_edge_metadata_title(G: nx.DiGraph, meta_key: str = "metadata", title_key: str = "title") -> int:
    ...
    try:
        ...
        log_error("clean_edge_metadata_title", e)
    return changed
...
def dedup_by_chunk_ids(
    ...
    try:
        ...
        log_error("dedup_by_chunk_ids", e)
        return chunk_ids, chunks, titles
...
def load_chunk_documents(chunk_dir: str, chunk_glob: str = "*chunks_clean.csv") -> Dict[str, Dict[str, Any]]:
    ...
    try:
        ...
        if not csv_files:
            log_error("load_chunk_documents", f"No chunk CSV files found in {chunk_dir!r} with pattern {chunk_glob!r}")
        ...
        for path in csv_files:
            try:
                ...
                log_error("load_chunk_documents", e, extra=f"path={path}")
                continue
        ...
        log_error("load_chunk_documents", e, extra=f"chunk_dir={chunk_dir}")
...
def embedding_function(
    ...
    **kwargs,
    ...
    if GRAPH_RAG_RESOURCES is None:
        log_error("embedding_function", "GRAPH_RAG_RESOURCES is not initialized")
        return []

    try:
        from GraphReasoning import generate_node_embeddings
    except Exception as e:
        log_error("embedding_function", e, extra="failed importing GraphReasoning.generate_node_embeddings")
        return []
    ...
    try:
        ...
        log_error("embedding_function", e)
        return []
...
def graph_rag_generate(
    ...
    if GRAPH_RAG_RESOURCES is None or GRAPH_RAG_RESOURCES.client is None or not GRAPH_RAG_RESOURCES.model:
        log_error("graph_rag_generate", "GRAPH_RAG_RESOURCES not fully initialized")
        return None
    ...
    try:
        ...
        log_error("graph_rag_generate", e)
        return None
...
def find_shortest_path_subgraph_between_nodes(
    ...
    try:
        ...
        log_error("find_shortest_path_subgraph_between_nodes", e)
        return graph.subgraph([]).copy()
...
class KnowledgeBase:
    def __init__(
        ...
        *,
        ...
        try:
            ...
            log_error("KnowledgeBase.__init__", e)
    ...
    def _embed_nodes(self, node_ids: List[str]) -> None:
        try:
            ...
            log_error("KnowledgeBase._embed_nodes", e)
    ...
    def similar_nodes(
        ...
        try:
            ...
            q_vecs = self.embed_fn([keyword])
            if not q_vecs:
                return []

            k = int(top_k or self.top_k)
            q = _l2_normalize(q_vecs[0])
            ...
            log_error("KnowledgeBase.similar_nodes", e, extra=f"keyword={keyword}")
            return []
    ...
    def extract_keywords(self, query: str) -> List[str]:
        try:
            ...
            log_error("KnowledgeBase.extract_keywords", e, extra=f"query={query}")
            return []
    ...
    def keywords_to_subgraph(
        ...
        try:
            ...
            log_error("KnowledgeBase.keywords_to_subgraph", e, extra=f"keywords={keywords}")
            return self.G.subgraph([]).copy()
...
def init_context(
    *,
    ...
    try:
        ...
        log_error("init_context", e)
        shared_context.knowledgebase = None
        shared_context.chunk_docs_by_id = {}
        shared_context.verbose = bool(verbose)
...
def graph_source_rag(
    ...
    try:
        if not shared_context.knowledgebase:
            log_error("graph_source_rag", "knowledgebase is not initialized")
            return ""
        ...
        if not paths_list:
            log_error("graph_source_rag", "no relations found", extra=f"query={query}")
            return ""
        ...
        for _, _, data in subgraph.out_edges(data=True):
            ...
            if not shared_context.chunk_docs_by_id:
                log_error("graph_source_rag", "chunk_docs_by_id is not initialized", extra=f"query={query}")
                return ""
        ...
        if not chunk_lines:
            log_error("graph_source_rag", "no usable source chunk contents found", extra=f"query={query}")
            return ""
        ...
        log_error("graph_source_rag", e, extra=f"query={query}")
        return ""
...
def initialize_graph_rag(
    *,
    ...
    try:
        ...
        log_error("initialize_graph_rag", e, extra="failed importing transformers/GraphReasoning")
    ...
    try:
        ...
        try:
            ...
            log_error("initialize_graph_rag", e, extra="pagerank failed")
        ...
        with torch.no_grad():
            if regenerate_embeddings:
                log_info("Regenerating graph node embeddings...")
                ...
                try:
                    ...
                    log_error("initialize_graph_rag", e, extra="save_embeddings failed")
            else:
                log_info(f"Loading graph node embeddings from: {embedding_cache_path}")
                try:
                    node_embeddings = load_embeddings(embedding_cache_path)
                except Exception as e:
                    log_error("initialize_graph_rag", e, extra="load_embeddings failed")
                    node_embeddings = {}
        ...
        GRAPH_RAG_RESOURCES = GraphRAGResources(
            ...
            graph=G,
            node_embeddings=node_embeddings if isinstance(node_embeddings, dict) else {},
        ...
        log_info(f"Graph RAG initialized: {len(G.nodes)} nodes, {len(G.edges)} edges, {len(chunk_docs_by_id)} chunks")
    except Exception as e:
        log_error("initialize_graph_rag", e)
        GRAPH_RAG_RESOURCES = GraphRAGResources(
            client=None,
            model=None,
            graph=None,
...
def teacher_generate_question(
    ...
    try:
        ...
        log_error("teacher_generate_question", e)
        return None
...
def teacher_generate_structured_answer(
    ...
    try:
        ...
        log_error("teacher_generate_structured_answer.graph_rag", e, extra=f"question={question}")
        rag_text = ""
        has_rag = False
...
"""
        ...
        log_error("teacher_generate_structured_answer", e, extra=f"question={question}")
        return None
...
"""
    try:
        ...
        log_error("teacher_generate_rejected", e, extra=f"question={question}")
        return None
...
"""
        ...
        log_error("validate_and_repair_graph", e)
...
def extract_graph_json_block_debug(
    chosen: str,
    client: Optional[OpenAI] = None,
    model: Optional[str] = None,
    ...
    try:
        inner = find_tag_block(chosen, GRAPH_JSON_START, GRAPH_JSON_END)
        if not inner:
            return None, "missing_graph_json_block"
        ...
        try:
            ...
            if client is not None and model is not None:
                graph = validate_and_repair_graph(client, model, inner)
                if graph is not None:
                    return graph.model_dump_json(), "repaired_ok"
            return None, f"json_load_failed: {e}"

        validated = validate_graph_semantics(obj)
        if validated:
            return json.dumps(validated, ensure_ascii=False), "ok"

        if client is not None and model is not None:
            graph = validate_and_repair_graph(client, model, inner)
            if graph is not None:
                return graph.model_dump_json(), "repaired_ok"
            return None, "validation_failed_and_repair_failed"

        return None, "validation_failed"
    except Exception as e:
        return None, f"extract_exception: {e}"
...
def build_graph_reasoning_dataset(
    ...
    for idx in indices:
        ...
        try:
            ...
            if not q:
                log_error("build_graph_reasoning_dataset", "teacher_generate_question failed", extra=f"corpus_idx={idx}")
                skipped += 1
                continue

            structured = teacher_generate_structured_answer(teacher_client, teacher_model, q, context)
            if not structured:
                log_error("build_graph_reasoning_dataset", "teacher_generate_structured_answer failed", extra=f"corpus_idx={idx}, question={q}")
                skipped += 1
                continue

            graph_json_str, graph_json_reason = extract_graph_json_block_debug(
                structured,
                client=teacher_client,
                model=teacher_model,
            )
            if not graph_json_str:
                print("\n[extract_graph_json_block] FAILED")
                print(f"reason: {graph_json_reason}")
                print(f"question: {q}")
                print("----- structured output preview -----")
                print(structured[:4000])
                print("----- end preview -----")
                skipped += 1
                continue
            ...
            if not final_answer:
                log_error("build_graph_reasoning_dataset", "extract_final_answer failed", extra=f"corpus_idx={idx}, question={q}")
                skipped += 1
                continue

            rejected = teacher_generate_rejected(reject_client, reject_model, q, context)
            if not rejected:
                log_error("build_graph_reasoning_dataset", "teacher_generate_rejected failed", extra=f"corpus_idx={idx}, question={q}")
                skipped += 1
                continue
            ...
            if len(rows) - last_save_count >= save_steps:
                ...
                try:
                    ...
                    log_error("build_graph_reasoning_dataset.checkpoint_save", e)
                ...
                if push_to_hub and output_repo:
                    try:
                        ...
                        log_error("build_graph_reasoning_dataset.checkpoint_push", e)
        except Exception as e:
            log_error("build_graph_reasoning_dataset.loop", e, extra=f"corpus_idx={idx}")
            skipped += 1
            continue
...
def main():
    parser = argparse.ArgumentParser(description="Build advanced graph-native reasoning dataset with non-blocking error handling.")
    ...
    if len(corpus) == 0:
        log_error("main", "Corpus is empty. Nothing to do.")
        return
    ...
    try:
        ...
        log_error("main.reject_client", e)
        reject_client = teacher_client

    if args.push_to_hub and not args.output_repo:
        log_error("main", "--output_repo must be set if --push_to_hub is used")
    ...
    try:
        ...
        log_error("main.graph_rag_client", e)
        graph_rag_client = None
